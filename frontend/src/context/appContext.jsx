...
//register
  ...
  const updateUser = async (currentUser) => {
   ...
   const data = await fetch("http://localhost:5000/api/v1/auth/updateUser", {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
logoutUser()
  ...
  //edit jobhttp://localhost:5000/api/v1/auth/register
  const editJob =async () => {
    dispatch({ type: EDIT_JOB_BEGIN })
    try {
      const { position, company, jobLocation, jobType, status } = state
      const data = await fetch(`http://localhost:5000/api/v1/jobs/${state.editJobId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "Authorization":`Bearer ${state.token}`
...
clearAlert()
  ...
  const deleteJob = async(id) => {
    ...
    try {
      const data=   await fetch(`http://localhost:5000/api/v1/jobs/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        "Authorization":`Bearer ${state.token}`
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        createJob, getJobs, deleteJob, setEditJob, editJob, 
